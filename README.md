...
- 调试页：新增独立 `调试` tab，包含检测日志、精度校准、参数三个子页签；当前第一版读取现有采样、提醒记录、基线和灵敏度配置，不额外改动识别算法。
- 设置页：标准/灵敏两档灵敏度、抽帧间隔、真实/模拟检测切换确认、本机隐私模式、轻提示音、短振动、本地数据确认清空。
- UI：按 HarmonyOS 沉浸光感方向升级浅雾背景、玻璃卡片、主评分卡、核心指标卡、胶囊按钮和悬浮底栏；仅调整视觉层，不改变功能入口、识别算法、提醒规则和采样链路。
