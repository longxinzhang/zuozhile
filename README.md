...
- 校准：`记录基线` 会采集 10 帧，通过人脸、双肩和头肩关键点质量检查后，按字段取中位数保存 pitch/roll/yaw、人脸框、肩宽、鼻肩比和嘴部比例；样本不足时不覆盖旧基线。设置页展示基线状态、最近记录时间、短格式基线值，以及 3 秒倒计时、10 帧采样进度和完成/失败提示；恢复默认基线前会二次确认。
