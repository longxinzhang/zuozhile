...
function load(file, dependencies, store) {
...
function fixture({ native = false, declared = true, granted = true, fail = false } = {}) {
  ...
  const controller = {
    ...
    setUIContext: async page => { assert.equal(page, 'pages/PostureFloat'); },
    start: async () => { calls.push('native-start'); if (fail) throw new Error('not supported'); },
    stop: async () => { calls.push('native-stop'); stateChange({ state: 'stopped' }); }
  };
  const child = {
  ...
  const service = load('PostureFloatService.ets', {
    '@kit.AbilityKit': { bundleManager: {
      ...
      requestPermissionsFromUser: async () => ({ authResults: [granted ? 0 : -1] })
    }) } },
    ...
    '@kit.ArkUI': { display: { getDefaultDisplaySync: () => ({ width: 1320, height: 2760, densityPixels: 3.5 }) },
      floatView: { isFloatViewEnabled: () => true, create: async () => { calls.push('native-create'); return controller; },
        FloatViewTemplateType: { ROUNDED_RECTANGLE: 0 }, FloatViewState: { STARTED: 'started', STOPPED: 'stopped' } } }
  }, store).PostureFloatService;
  service.initialize({ createSubWindow: async () => child });
  return { service, store, calls, started: () => stateChange({ state: 'started' }) };
}
...
async function verify() {
  ...
  console.log('PASS: child-window fallback, cosmetic failure, permission denial, native callbacks, duplicate open, early close, start failure, main-window brightness restoration');
